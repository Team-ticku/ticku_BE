...
router.get("/:corpCode", async (req, res) => {
  ...
  try {
    ...
    for (const filing of recentFilings) {
      ...
      if (
        ...
        cfsData.forEach((item) => {
          if (
            item.account_nm.includes("자산") &&
            item.account_nm.includes("총계")
          ) {
            data["자산총계"] = parseAmount(item.thstrm_amount);
          } else if (
            item.account_nm.includes("부채") &&
            item.account_nm.includes("총계")
          ) {
            data["부채총계"] = parseAmount(item.thstrm_amount);
          } else if (item.account_nm.includes("자본총계")) {
            data["자본총계"] = parseAmount(item.thstrm_amount);
          }
          // else if (item.account_nm.includes("매출액")) {
    ...
    reports.sort((a, b) => a.reportYear.localeCompare(b.reportYear));
    res.json(reports);
  } catch (error) {
